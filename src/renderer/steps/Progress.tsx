...
export const Progress = memo(function Progress({
  ...
  return <div className="page progress-page">
      ...
      <div className="statuses">
        {platforms.map(platform => <div key={platform} className={`button progress-button ${done.includes(platform) ? "progress-button-done" : ""} ${error.includes(platform) ? "progress-button-error" : ""}`}>
            <span>{PLATFORM_LABELS[platform]}</span>
          </div>)}
      </div>
      {hasError && <div className="progress-note">
      ...
      {allDone && <div className="progress-bottom">
          {hasError ? <button className="button button-secondary" onClick={run}>
              Try Again
            </button> : <button className="button button-secondary" onClick={async () => {
        await init(true);
        navigate("/action");
      }}>
              Start Over
